...
// POST /create ann
app.post("/admin_login/create", (req, res) => {
    const userID = req.session.userID;
    console.log(userID);
    let currentDate = new Date();
    const newAnn = {
        "title": req.body.title, "img": req.body.img || defaultImg, "comment": req.body.comment, "user": req.session.userID,
...
// POST /edit/:annID
app.post("/admin_login/edit/:id", (req, res) => {
    const id = req.params.id;
    let currentDate = new Date();
    // const book = [req.body.title, req.body.author, req.body.comment, id];
    const announcement = {
        "title": req.body.title, "img": req.body.img || defaultImg, "comment": req.body.comment, "annID": id, "user": req.session.userID,
        "dateOfAnn": currentDate.getDate() + "-" + (currentDate.getMonth() + 1) + "-" + currentDate.getFullYear() + " " +
            currentDate.getHours() + ":" + currentDate.getMinutes()
    }
...
// POST /create animal
app.post("/admin_login/createAnimal", (req, res) => {
    const userID = req.session.userID;
    console.log(userID);
    let currentDate = new Date();
    const newAnimal = {
        "animalName": req.body.animalName, "animalImg": req.body.animalImg, "animalText": req.body.animalText, "animalUser": req.session.userID,
...
// POST /edit/:animalID
app.post("/admin_login/editAnimal/:animalID", (req, res) => {
    const id = req.params.animalID;
    let currentDate = new Date();
    const animal = {
        "animalName": req.body.animalName, "animalImg": req.body.animalImg, "animalText": req.body.animalText, "animalID": id, "animalUser": req.session.userID,
        "animalDate": currentDate.getDate() + "-" + (currentDate.getMonth() + 1) + "-" + currentDate.getFullYear() + " " +
            currentDate.getHours() + ":" + currentDate.getMinutes(), "animalType": req.body.animalType
    }
